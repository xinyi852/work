...
type GoLimit struct {
	...
	running   map[uint64]int8  //运行中的任务ID
...
func NewGoLimit(max uint) *GoLimit {
	return &GoLimit{max: max, count: 0, isAddLock: false, running: make(map[uint64]int8), zeroChan: nil}
}

// Add 并发计数加1.若 计数>=max_num, 则阻塞,直到 计数<max_num
func (g *GoLimit) Add(id uint64) {
	g.addLock.Lock()
	g.dataLock.Lock()
...
// IsRunning 是否在运行中
func (g *GoLimit) IsRunning(id uint64) bool {
	_, ok := g.running[id]
	return ok
}
